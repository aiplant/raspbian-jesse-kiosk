...
"
  ...
  printf "\t /etc/xdg/lxsession/LXDE-pi/autostart \n"
  printf "\t I will add the$GREEN GREEN$OFF lines to the file...\n\n"

  printf "$DIM\n"
  pr -t -o 9 /etc/xdg/lxsession/LXDE-pi/autostart
  ...
  cat /etc/xdg/lxsession/LXDE-pi/autostart > /etc/xdg/lxsession/LXDE-pi/autostart.`date +%m-%d-%Y_%I:%M:%S`
  printf "\n\t Created backup copy of the autostart file...\n"
  printf "$NOBLANK" >> /etc/xdg/lxsession/LXDE-pi/autostart
